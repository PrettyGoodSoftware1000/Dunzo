...
- **Export / Import** — one click downloads a `.json` backup (re-importable, restores everything including whiteboards) and a human-readable `.rtf` with whiteboard text and images embedded. Images are written in both modern (JPEG/PNG) and legacy (WMF bitmap) RTF forms for maximum compatibility — note that macOS **TextEdit never shows images in plain RTF files** by design; open the file in Word, Pages, LibreOffice, or Google Docs to see them.
